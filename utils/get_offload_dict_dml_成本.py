import numpy as np
import pickle
from utils.graph_builder_成本 import GraphBuilder
    ...
    try:
        ...
        for Ai in Ai_range:
            ...
            for sim in range(num_simulations):
                # cifar10数据集的最低容量用15，mnist数据集最低容量用5
                ...
                # 修改：接收完整返回值
                throughput_feasible, cost_feasible, cost_efficiency_feasible, worker_training_cost,offloading_total_cost, worker_result_to_server_cost,server_to_sink_cost,ai_actdata_sub, training_data = solver.solve(print_training_data=True)
                ...
                print(f"offloading_total_cost: {offloading_total_cost}")
                print(f"worker_result_to_server_cost: {worker_result_to_server_cost}")
                print(f"server_to_sink_cost: {server_to_sink_cost}")
