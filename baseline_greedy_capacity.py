"""
基线方法 4: Greedy Capacity-based Heuristic (贪心容量启发式)
- 基于设备容量指标贪心选择工作节点
...
原理:
  每 T 时隙对每个设备计算贪心得分:
    score_i = F_i(t) / e_device_i(t)
    即: 计算能力 / 能量消耗
  按得分降序选择 top-K 个设备作为工作节点。

这个基线回答了审稿人 1.4 中要求的 "one greedy capacity-based heuristic"。
...
def get_real_flow_mb_in_t_time(total_slots=None, onlineFL_T=None, p_value=20, A_min=30, cap_min=20):
    """
    生成 Greedy Capacity 基线的时隙结果。

    策略:
    - 每 T 时隙: 计算每设备的容量得分 score_i = F_i(t) / e_device_i(t)
    - 贪心选择 top-K 个得分最高的设备
    ...
    s_t_greedy = np.zeros((N, 1))
    Result = np.zeros(1)

    # ============ 主仿真循环 ============
    for Time in range(1, Num_slot + 1):
        if Time % 200 == 0:
            print(f"[Greedy Capacity] Time = {Time}")
        ...
        # ---- 大时间尺度: 贪心容量选择 ----
        if (Time - 1) % T == 0:
            # 贪心得分: 计算能力 / 单位能耗 = F_i / e_device_i
            scores = (F_t.flatten() / (e_device_t.flatten() + 1e-8))
            # 按得分降序选择 top-K
            top_indices = np.argsort(scores)[::-1][:K]
            s_t_greedy = np.zeros((N, 1))
            s_t_greedy[top_indices] = 1
        ...
        for i in range(1, N + 1):
            cap_recons[i, N + 1] = s_t_greedy[i - 1] * F_t[i - 1]
        ...
        lambda_val = np.maximum(lambda_val + epsilon * (
            s_t_greedy * e_device_t +
            np.sum(flow_temp[1:N + 1, 1:N + 1] * e_link_t, axis=1, keepdims=True) - E_t
        ), 0)

        # ---- 提取结果 ----
        selected_devices_t = np.where(s_t_greedy.flatten() > 0.5)[0].tolist()
        total_offload_t = 0.0
        for i in range(1, N + 1):
